...
const handler = async (event, context) => {
    // Parse body and return error if the JSON format is wrong
    let body = undefined;
    try {
    ...
    console.log(body);
    // Check fields and return errors if there is improper formatting
    const url = body.URL;
    const content = body.Content;
    var debloat = body.debloat;
    const preVersion = body.Version;
    ...
    try {
        ...
        // URL Proceedure
        if (url != undefined) {
            return urlExtract(url, dir, debloat, preVersion);
        }
        // Content Proceedure
        else {
            ...
            return contentExtract(content, dir, Name, debloat, preVersion);
        }
    }
    catch {
        return Err400;
...
async function urlExtract(testurl, dir, debloat, preVersion) {
    ...
    try {
        ...
        if (preVersion != undefined && preVersion != "" && (0, s3_repo_1.checkValidVersion)(preVersion)) {
            version = preVersion;
        }
        else if ("version" in packageJson && (0, s3_repo_1.checkValidVersion)(packageJson.version)) {
...
async function contentExtract(content, dir, Name, debloat, preVersion) {
    // Create the buffer
